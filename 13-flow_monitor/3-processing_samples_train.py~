...
with open("owd-new.txt") as f:
    for l in f:
        ...
        if float(l[1]) >= 0.140:  # if more than expected value
            d.append(00.120000)
            d_rm.append(l[1])
        elif float(l[1]) <= 0.030:
...
sample_to_be_consider = 200	# its multiplication factor of 20 So, 20*200=4000
...
sample_to_be_consider_iat = 200	# its multiplication factor of 20 So, 20*200=4000
...
lines = []
with open('iat-results') as infile:
    for line in infile:
        for src, target in replacements.iteritems():
            line = line.replace(src, target)
        lines.append(line)
#print lines
with open('iat-results', 'w') as outfile:
    for line in lines:
        outfile.write(line)
...
plt.title("13 - Flow, Queue=100, RTT=60ms, BW=20Mbps", fontsize=13)	# set titels, x and y label
plt.xlabel('Time (sec)', fontsize=12)
plt.ylabel('One-way Delay (sec)', fontsize=12)

plt.plot(graph, delay,'bx', ls="", ms=3,label='Measured OWD')
#plt.plot(graph, delay, linewidth=0.5, label='Measured OWD')	# plot line graph between graph and delay
#plt.legend(bbox_to_anchor=(1, 1), loc=1, borderaxespad=0)
plt.xticks(np.arange(min(graph), max(graph), 20.0))	# set the current tick locations and labels of the x-axis
#plt.yticks(np.arange(0, 0.21, step=0.03))
#plt.yticks(min(delay), max(delay)+0.02, 0.02)
plt.grid(True)
...
plt.legend()
plt.savefig('OWD.png', figsize=(30,15), dpi = 300)
#plt.savefig('OWD.pdf')
pyplot.show()
...
plt.title("13 - Flow, Queue=100, RTT=60ms, BW=20Mbps", fontsize=13)	# set titels, x and y label
plt.xlabel('Time (sec)', fontsize=12)
plt.ylabel('Inter-Arrival Time (sec)', fontsize=12)

#plt.plot(iat_s4_graph_np, iat_s4_np,linewidth=0.5)	# plot line graph between graph and delay
plt.plot(iat_s4_graph_np, iat_s4_np,'bx', ls="", ms=3,label='Measured IAT')
...
#plt.yticks(np.arange(-0.001, 0.030, step=0.003))
plt.grid(True)
plt.plot(iat_s4_graph_np[win_index_iat],iat_s4_np[win_index_iat],'ro', ls="", ms=3,label='Training Samples - IAT')	# mark congestion points in RED
...
plt.legend()
plt.savefig('iat.png', figsize=(30,15), dpi = 300)
#plt.savefig('iat.pdf')
pyplot.show()
